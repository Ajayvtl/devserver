...
export async function getDevCenterSection(slug: string): Promise<DevCenterSectionData> {
  return requestOrFallback<DevCenterSectionData>(`/api/devcenter/${encodeURIComponent(slug)}`, devcenterFallback(slug))
}

export async function listDevCenterSections(): Promise<Array<{ key: string; title: string; subtitle: string }>> {
  return requestOrFallback<Array<{ key: string; title: string; subtitle: string }>>('/api/devcenter', [
